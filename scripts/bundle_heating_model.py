...
class BundleHeatingModel(object):
    ...
    def __init__(self, heating_options, field, num_bins=50, footpoints=None):
        self.field = field
    ...
    def calculate_event_properties(self, loop):
        ...
        rates = ((self.heating_options['stress']**2) * (bundle.storm_fluxes / bundle.number_loops) 
                 / loop.full_length.to(u.cm) / (self.heating_options['event_duration'] / 2.)) 
        
        return {'magnitude': rates.value,
                'rise_start': start_times.to(u.s).value,
                'rise_end': (start_times + self.heating_options['event_duration']/2.).to(u.s).value,
                'decay_start': (start_times + self.heating_options['event_duration']/2).to(u.s).value,
                'decay_end': (start_times + self.heating_options['event_duration']).to(u.s).value,}
    ...
    def constrain_number_storms(self, total_time, number_storms=100, flux_constraint=1e7*u.erg/u.cm**2/u.s,
        ...
        while error > error_tolerance and tries < max_tries:
            # Get storm times
            storm_times = np.random.uniform(low=0., high=total_time.to(u.s).value, size=number_storms) * u.s
            # Create bundles
    ...
    def create_bundles(self, footpoints, num_bins,):
        ...
        for i_x, i_y in np.stack(np.where(hist != 0), axis=1):
            ...
            top_right_corner = SkyCoord(bin_edges_x[i_x+1]*u.arcsec, bin_edges_y[i_y+1]*u.arcsec,
                                        frame=self.field.magnetogram.coordinate_frame)
            bundles.append(Bundle(loops, bottom_left_corner, top_right_corner, self.heating_options['storm_duration']))
