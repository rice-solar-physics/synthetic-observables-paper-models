...
\end{equation*}
where $\Delta t$ is the timestep, $j$ indexes time, and $\mathcal{O}(\Delta t^2)$ denotes terms of order 2 or higher in $\Delta t$. Dropping the higher order terms and using \autoref{eq:nei_mat} yields an expression for $\mathbf{F}_{j+1}$,
...
\end{equation*}
is the mean-subtracted and scaled intensity of channel $c$ as a function of time. Taking the Fourier transform of both sides of \autoref{eq:cc_pre} and using the convolution theorem gives,
\begin{align*}
